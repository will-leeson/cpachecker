...
class GGNN(nn.Module):
    ...
    def __init__(self, passes, numEdgeSets):
        ...
        self.fcLast = nn.Linear(80, 6)

    def forward(self, nodes, backwards_edge, problemType):
        for j in range(self.passes):
            incoming = torch.zeros_like(nodes)
            counter = 0
            for edgeSet in backwards_edge:
                try:
                    y = self.edgeNets[counter][0](nodes[edgeSet[:,1]])
                    y = f.leaky_relu(y)
                ...
                counter+=1
            nodes = self.gru(incoming, nodes)
            if j < self.passes - 2:
                nodes = nodes.detach()
        
        nodes = nodes.sum(dim=0)
        nodes = torch.log(nodes)
        nodes[torch.isnan(nodes)] = 0
        nodes = f.relu(nodes)
        nodes[nodes==float("inf")] = nodes[nodes!=float("inf")].max()
        
        x = torch.cat((nodes, problemType))
        x = self.fc1(x)
        x = f.leaky_relu(x)
        x = self.fc2(x)
        x = f.leaky_relu(x)
